...
and [2,5,6]. The result of the merge is [1,2,2,3,5,6] with the underlined elements coming from
nums1. Example 2:
...
Input: nums1 = [0], m = 0, nums2 = [1], n = 1
                                Output: [1]
                                         Explanation: The arrays we are merging are [] and [1].
                                                       The result of the merge is [1].
                                                       Note that because m = 0, there are no
...
public:
    void
    merge(std::vector<int>& nums1, int nums1Len, std::vector<int>& nums2, int nums2Len)
    {
        std::ranges::copy(nums2.begin(), nums2.end(), nums1.begin() + nums1Len);
        std::inplace_merge(nums1.begin(), nums1.begin() + nums1Len, nums1.end());
    }
};
...
main()
{
    Solution solution {};
    {
        ...
        solution.merge(nums1, nums1Len, nums2, nums2Len);
    }
