...
public:
    int
    findTheDistanceValue(std::vector<int>& arr1, std::vector<int>& arr2, int distance)
    {
        int numOfValidElems = 0;
        for (const auto val1 : arr1)
        {
            bool isDistanceValid {true};
            for (const auto val2 : arr2)
            {
                if (std::abs(val1 - val2) <= distance)
                {
